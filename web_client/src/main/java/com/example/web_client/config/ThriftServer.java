...
import com.example.web_client.service.ThriftRpcServiceImpl;
...
public class ThriftServer {
    ...
    //业务服务对象
    @Autowired
    ThriftRpcServiceImpl thriftRpcService;
    ...
    public void start() {
        try {
            ...
            //泛型就是实现的业务
            ThriftRpcService.Processor<ThriftRpcServiceImpl> processor = new ThriftRpcService.Processor<>(thriftRpcService);
            ...
            log.info("thrift server started at port: {}", port);
            // TODO 异步非阻塞的死循环
            new Thread(server::serve).start();
        } catch (TTransportException e) {
            log.error("ThriftServer start error: ", e);
        }
    }
}
